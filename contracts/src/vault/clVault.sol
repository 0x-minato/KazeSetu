...
import { KazeCommon } from "../common/kazeCommon.sol";
import { ERC20Upgradeable } from "@openzeppelin/contracts-upgradeable/token/ERC20/ERC20Upgradeable.sol";
import { IERC20 } from "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import { IKazeClVault } from "./interfaces/IKazeClVault.sol";
import { ZeroAddress, ZeroAmount, BrokenInvariant, InvalidTicks, InvalidTokens, InvalidFee, ZeroLiquidity } from "../errors/Errors.sol";
...
contract KazeClVault is KazeCommon, IKazeClVault, ERC20Upgradeable {
    ...
    function _process_deposit(
        ...
        (liquidityAdded, amount0Used, amount1Used) = _add_liquidity_uniswap(
            pool_data.tokenId, position_.tickLower, position_.tickUpper, a0, a1
        ); 
    ...
    function _add_liquidity_uniswap(
        ...
        if (token_id == 0) {
            ...
            position.tokenId = token_id_new.toUint48();
        } else {
    ...
    function _get_pool_data(PositionConfig memory position_) internal view returns (PoolData memory pool_data) {
        ...
        pool_data = PoolData({
            ...
            tokenId: position_.tokenId
        });
    }

    function _get_price(uint256 sqrt_priceX96) internal pure returns(uint256 price) {
        price = Math.mulDiv(
            Math.mulDiv(sqrt_priceX96, sqrt_priceX96, 1 << 96),
            PRICE_SCALE,
            1 << 96
        );
    }
